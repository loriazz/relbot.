...
exports.run = async (client, message, args) => {//hamzamertakbaba#3575
  ...
  let clear = new MessageButton().setLabel('Clear').setID(calculator_ac).setStyle('red')
  ...
  let qac = new MessageButton().setLabel('Clear').setID(calculator_ac).setStyle('red').setDisabled()
    ...
    calc.on('collect', async btn => {
      ...
      if (btn.id === calculator_1) {
        ...
        try {
          ...
          message.channel.send(`${message.author} Geçersiz bir işlem girdin. Sol üstte bulunan \`Clear\` butonuna basarak hesaplamayı sıfırla.`).then((m) => m.delete({ timeout: 10000 }))
