...
exports.run = async (client, message, args) => {
        	 ...
        	 let karaliste = db.get(`./mainjsons/ckaraliste.${message.author.id}`)
 const westraben = new Discord.MessageEmbed()
 .setColor("#f6ff00")
 .setDescription(` **${karaliste}** sebebiyle karalisteye alınmışsın!\nBeyaz listeye alınmak istiyorsan [BURAYA]() gelebilirsin!`)
  if(karaliste) 
    return message.channel.send(westraben)

let kanal = await db.get(`./mainjsons/seviyekanal${message.guild.id}`)

let xp = await db.get(`./mainjsons/seviyexp${message.guild.id}`)

let sınır = await db.get(`./mainjsons/seviyesınır${message.guild.id}`)
