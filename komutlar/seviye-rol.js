...
.addField(`Ayarlanan Rolleri Görmek:`, `\`${p}seviyerolleri\``)) 
      ...
      if(args[0] == 'temizle' || args[0] == "clear") {
        ...
        db.delete(`./mainjsons/srol.${message.guild.id}`)

        db.delete(`./mainjsons/srol2.${message.guild.id}`)

        db.delete(`./mainjsons/srol3.${message.guild.id}`)
...
return message.channel.send(embed)
      ...
      db.push(`./mainjsons/srol.${message.guild.id}`, role.id)

      db.set(`./mainjsons/srol2.${message.guild.id}.${role.id}`, args[1])

      db.push(`./mainjsons/srol3.${message.guild.id}.${role.id}`, args[1])
